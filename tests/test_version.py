...
"2.1.0" literal, a real packaging.version compare (so 2.10.0 > 2.9.0 and a
...
def test_version_literal_is_2_1_0():
    """The single literal is exactly "2.1.0" (PEP 440, no leading v)."""
...
def test_parse_defaults_to_module_version():
    ...
    assert parse_version() == parse_version("2.1.0")


def test_version_tuple_pads_release_to_four_ints():
    """version_tuple() -> (2, 1, 0, 0): release padded with 0 to length 4."""
    assert version_tuple() == (2, 1, 0, 0)
